...
export function filterDuplicates(proposed, sentHistory, cooldownMs = 24 * 60 * 60 * 1000 // Default 24 hour cooldown
) {
    const now = Date.now();
...
export function evaluateAssessmentDeadlines(assessments, now) {
    const notifications = [];
    const FORTY_EIGHT_HOURS_MS = 48 * 60 * 60 * 1000;
    for (const asst of assessments) {
        if (!asst.dueDate || asst.isCompleted)
            continue;
...
export function evaluateRiskElevations(riskDeltas, now) {
    const notifications = [];
    for (const delta of riskDeltas) {
        if ((delta.current === 'HIGH' || delta.current === 'CRITICAL') &&
