...
const evaluationRoutes = async (app) => {
    const evaluationService = createEvaluationService(app.prisma);
    app.post("/api/v1/subjects/:subjectId/evaluation-plan", { preHandler: app.authenticate }, async (request, reply) => {
        const key = requireIdempotencyKey(request);
        const { subjectId } = request.params;
        const body = CreatePlanSchema.parse(request.body);
        const { statusCode, body: responseBody, replayed } = await withIdempotency(app.prisma, {
            userId: request.userId,
            key,
            route: `POST /api/v1/subjects/${subjectId}/evaluation-plan`,
        }, async () => {
            const plan = await evaluationService.createEvaluationPlan(request.userId, subjectId, body.components);
    ...
    app.get("/api/v1/subjects/:subjectId/evaluation-plan", { preHandler: app.authenticate }, async (request, reply) => {
        const { subjectId } = request.params;
    ...
    app.post("/api/v1/assessments", { preHandler: app.authenticate }, async (request, reply) => {
        const key = requireIdempotencyKey(request);
        const body = CreateAssessmentSchema.parse(request.body);
        const { statusCode, body: responseBody, replayed } = await withIdempotency(app.prisma, { userId: request.userId, key, route: "POST /api/v1/assessments" }, async () => {
    ...
    app.patch("/api/v1/assessments/:assessmentId/marks", { preHandler: app.authenticate }, async (request, reply) => {
        const key = requireIdempotencyKey(request);
        ...
        const { statusCode, body: responseBody, replayed } = await withIdempotency(app.prisma, {
            ...
            const result = await evaluationService.updateMarks(request.userId, assessmentId, body.obtainedMarks);
            return { statusCode: 200, body: { requestId: request.requestId, data: result } };
        });
        reply.header("Idempotency-Replayed", String(replayed)).code(statusCode).send(responseBody);
    });
    app.get("/api/v1/grades/sgpa-scenarios", { preHandler: app.authenticate }, async (request, reply) => {
        const query = SgpaQuerySchema.parse(request.query);
        const targets = query.targets ? JSON.parse(query.targets) : {};
    ...
    app.get("/api/v1/risk", { preHandler: app.authenticate }, async (request, reply) => {
    ...
    app.post("/api/v1/risk/recompute", { preHandler: app.authenticate }, async (request, reply) => {
        const key = requireIdempotencyKey(request);
        const body = RecomputeRiskSchema.parse(request.body);
        const { statusCode, body: responseBody, replayed } = await withIdempotency(app.prisma, { userId: request.userId, key, route: "POST /api/v1/risk/recompute" }, async () => {
