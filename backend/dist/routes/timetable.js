...
const timetableRoutes = async (app) => {
    ...
    app.post("/api/v1/timetable/import", { preHandler: app.authenticate }, async (request, reply) => {
        const key = requireIdempotencyKey(request);
        const body = ImportTimetableSchema.parse(request.body);
        const { statusCode, body: responseBody, replayed } = await withIdempotency(app.prisma, { userId: request.userId, key, route: "POST /api/v1/timetable/import" }, async () => {
            const result = await timetableService.importEntries(request.userId, body.semesterId, body.entries);
