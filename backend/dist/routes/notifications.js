import { z } from 'zod';
import { evaluateAssessmentDeadlines, evaluateRiskElevations, filterDuplicates } from '../modules/notifications/notifications.engine.js';
...
export default async function notificationsRoutes(app) {
    app.get('/api/v1/notifications', async (request, reply) => {
        ...
        const activeAssessments = assessmentsData.map(a => ({
            id: a.id,
            name: a.title,
        ...
        const sentHistory = sentHistoryData.map(n => ({
            id: n.id,
            type: n.type,
            severity: 'WARNING', // Default fallback
            title: n.title,
            message: n.message,
        ...
        const mapped = dbNotifs.map(n => {
            let priority = 'NORMAL';
            if (n.type === 'HIGH_RISK_CHANGE')
                priority = 'CRITICAL';
            else if (n.type === 'UPCOMING_ASSESSMENT')
                priority = 'HIGH';
            return {
                id: n.id,
                category: n.type,
    ...
    app.post('/api/v1/notifications/:id/dismiss', async (request, reply) => {
        const userId = request.userId;
    ...
    app.post('/api/v1/notifications/:id/snooze', async (request, reply) => {
        const userId = request.userId;
        const { id } = request.params;
