import type { PrismaClient, Weekday, SessionType } from "@prisma/client";
import {
...
export function createTimetableService(prisma: PrismaClient) {
  ...
  return {
    async importEntries(userId: string, semesterId: string, entries: TimetableEntryInput[]) {
      ...
      const byDay = new Map<string, TimetableEntryInput[]>();
        ...
        for (let i = 0; i < sorted.length - 1; i++) {
          if (sorted[i].endTime > sorted[i + 1].startTime) {
            throw AppError.validation(
              `Overlapping timetable entries on ${day}: "${sorted[i].subjectCode}" ` +
      ...
      for (const entry of entries) {
        ...
        if (!subject) {
          throw AppError.validation(
            `Unknown subject code "${entry.subjectCode}" in this semester.`,
        ...
        rows.push({
          ...
          sessionType: entry.sessionType ?? ("LECTURE" as SessionType),
        });
      }
