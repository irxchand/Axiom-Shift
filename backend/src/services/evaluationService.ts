...
} from "./evaluationLogic.js";
import { AppError } from "../lib/errors.js";

const DEFAULT_TARGET_PERCENT = 75;

type ComponentRow = { id: string; weightPercent: number };
type AssessmentRow = {
  id: string;
  evaluationComponentId: string;
  maxMarks: number;
  dueDate: Date | null;
  marks: { obtainedMarks: number | null } | null;
};

/** Shared by risk recompute and SGPA outlook — both need a subject's secured/remaining weight. */
async function getOverallScoreForSubject(
  client: PrismaClient,
  subjectId: string,
  userId: string,
): Promise<{ overall: OverallScoreResult; assessments: AssessmentRow[] } | null> {
  ...
  if (!plan) return null; // no plan yet — nothing to score

  const assessments: AssessmentRow[] = await assessmentRepo.listForSubject(subjectId, userId);

  const components = plan.components.map((component: ComponentRow) => ({
    id: component.id,
...
async function recomputeRiskForSubjectWithClient(
  client: PrismaClient,
  subjectId: string,
  userId: string,
) {
...
export function createEvaluationService(prisma: PrismaClient) {
  ...
  return {
    ...
    async getEvaluationPlan(userId: string, subjectId: string) {
      await requireOwnedSubject(subjectId, userId);
      const plan = await evaluationRepo.findPlanForSubject(subjectId, userId);
      if (!plan) throw AppError.notFound("No evaluation plan exists for this subject yet.");
      return plan;
    },
     ...
     */
    async updateMarks(userId: string, assessmentId: string, obtainedMarks: number | null) {
      ...
      return prisma.$transaction(async (tx) => {
        ...
        const riskResult = await recomputeRiskForSubjectWithClient(
          client,
    ...
    async getRiskForUser(userId: string) {
      return riskRepo.listForUser(userId);
    },

    async recomputeRisk(userId: string, subjectId: string) {
      await requireOwnedSubject(subjectId, userId);
     ...
     */
    async getSgpaScenarios(userId: string, targetPercentBySubject: Record<string, number>) {
      const semesters = await semesterRepo.listForUser(userId);
      const activeSemester = semesters[0];
      if (!activeSemester) throw AppError.notFound("No semester found for this user.");

      const subjects = await subjectRepo.listForSemester(activeSemester.id, userId);
      const gradedSubjects = subjects.filter(
        (s: { credits: number | null }) => s.credits !== null,
      ) as { id: string; credits: number | null }[];
