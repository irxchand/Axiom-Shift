...
export function expandCalendarEvents(
  ...
  for (const event of events) {
    ...
    if (!event.dayOfWeek || !event.startTime || !event.endTime) continue;
...
function toOccurrence(event: RawCalendarEvent, start: DateTime, end: DateTime): Occurrence {
  return {
    eventId: event.id,
...
export interface DaySummary {
  currentClass: Occurrence | null;
  nextClass: Occurrence | null;
  todaySchedule: Occurrence[];
...
export function computeDaySummary(
  ...
  now: DateTime,
  timezone: string,
  dayWindow: { startHour: number; endHour: number } = { startHour: 8, endHour: 20 },
  holidayDates: string[] = [],
): DaySummary {
  ...
  const lookaheadOccurrences = expandCalendarEvents(
    events,
    dayStart,
    lookaheadEnd,
    timezone,
...
export function isHoliday(date: DateTime, holidayDates: string[]): boolean {
 ...
 */
export function detectOverlaps(occurrences: Occurrence[]): OverlapPair[] {
  ...
  for (let i = 0; i < sorted.length; i++) {
    for (let j = i + 1; j < sorted.length; j++) {
      const a = sorted[i];
      const b = sorted[j];
      if (b.start >= a.end) break; // sorted by start — no further j can overlap a
