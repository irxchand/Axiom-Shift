...
export class DocumentHandoffService {
  ...
  async processHandoff(handoffId: string): Promise<any> {
    ...
    try {
      const result = await this.adapter.execute<{ sourceId: string }>({
        action: 'UPLOAD_FILE',
...
export async function processDocumentHandoff(handoffId: string) {
