...
const semesterRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/api/v1/semesters", { preHandler: app.authenticate }, async (request, reply) => {
    ...
    const { statusCode, body: responseBody, replayed } = await withIdempotency(
      app.prisma,
      { userId: request.userId, key, route: "POST /api/v1/semesters" },
      async () => {
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const { statusCode, body: responseBody, replayed } = await withIdempotency(
        app.prisma,
        { userId: request.userId, key, route: `POST /api/v1/semesters/${semesterId}/subjects` },
        async () => {
