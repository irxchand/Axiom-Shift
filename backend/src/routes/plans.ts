...
import { z } from 'zod';
import { PlanningService, PlanTaskProposal } from '../services/planningService.js';
import { rankSubjectsByUrgency, generateDeterministicReason, PlanningSubjectContext } from '../modules/planning/planning.fallback.js';
...
export default async function plansRoutes(app: FastifyInstance) {
  ...
  app.post('/api/v1/plans/generate', async (request, reply) => {
    ...
    const subjectsContext: PlanningSubjectContext[] = (semester.subjects || []).map((sub: any) => {
