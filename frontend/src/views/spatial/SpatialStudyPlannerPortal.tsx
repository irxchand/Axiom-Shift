...
export const SpatialStudyPlannerPortal: React.FC = () => {
  const { data: initialTasks = [], isLoading: isLoadingTasks, isError: isErrorTasks } = useQuery({ 
    queryKey: ['studyTasks'], 
  ...
  const { data: currentPlan, isLoading: isLoadingPlan } = useQuery({
    ...
    queryFn: backendAPI.getCurrentPlan
  });

  const generatePlanMutation = useMutation({
    mutationFn: backendAPI.generatePlan,
    onSuccess: () => {
  ...
  const acceptPlanMutation = useMutation({
    mutationFn: backendAPI.acceptPlan,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['currentPlan'] });
  ...
  if (isLoadingTasks || isLoadingPlan) {
    return <ParchmentCard className="p-8 text-center text-xs text-gold-foil animate-pulse">Loading Study Journal & Plans...</ParchmentCard>;
  }
    ...
    <div className="space-y-6 animate-fadeIn pb-24 text-[#d8cebe] font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
        ...
        <div className="lg:col-span-2 space-y-4 text-xs">
          <div className="flex justify-between items-center pb-2 border-b border-[#28211a]">
            ...
            <div className="flex gap-2">
              <button 
                ...
                className="px-3 py-1.5 bg-[#1b1612] hover:bg-[#28211a] text-[#c9a45c] text-[10px] font-bold border border-[#c9a45c]/30 rounded flex items-center gap-2 transition-all disabled:opacity-50"
              >
                {generatePlanMutation.isPending ? 'GENERATING...' : 'GENERATE AI PLAN'}
              </button>
              {currentPlan && currentPlan.status === 'PROPOSED' && (
                <button 
                  onClick={() => acceptPlanMutation.mutate(currentPlan.id)}
                  disabled={acceptPlanMutation.isPending}
                  className="px-3 py-1.5 bg-[#2a3c2a] hover:bg-[#354c35] text-emerald-300 text-[10px] font-bold border border-emerald-500/30 rounded flex items-center gap-2 transition-all disabled:opacity-50"
                >
                  {acceptPlanMutation.isPending ? 'ACCEPTING...' : 'ACCEPT PROPOSED PLAN'}
                </button>
              )}
            </div>
          </div>
          ...
          {currentPlan && currentPlan.status === 'PROPOSED' && (
            <div className="mb-4 p-4 border border-emerald-500/30 bg-[#2a3c2a]/20 rounded-xl space-y-3">
              <h3 className="text-xs font-bold text-emerald-400">PROPOSED PLAN: {currentPlan.title}</h3>
              <p className="text-[10px] text-[#9a9082]">Generated By: {currentPlan.generatedBy}</p>
              <div className="space-y-2 mt-2">
                {currentPlan.tasks?.map((pt: any) => (
                  <div key={pt.id} className="p-2 border border-[#28211a] bg-[#0f0c0a] rounded text-[10px]">
                    <strong className="text-[#f5ebe0]">{pt.title}</strong>
                    <p className="text-[#9a9082] mt-1">{pt.reason}</p>
                    <p className="text-[#c9a45c] mt-1 text-[9px]">{new Date(pt.startAt).toLocaleString()} - {new Date(pt.endAt).toLocaleString()}</p>
                  </div>
                ))}
              </div>
            </div>
